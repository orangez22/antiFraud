...
Page({
  data: {
    ...
    replyCommentId: null,  // 新增字段用于存储当前回复的评论ID
    replyContent: '',  // 回复内容
        ...
        if (errorCode === 20000) {
          ...
          const newComments = data.records.map((comment) => {
            return {
              ...
              showReplyBox: false,  // 初始化每个评论的回复框状态
  ...
  publishComment() {
    ...
    const memberId = getApp().getMemberId(); // 获取全局 memberId
    const role = getApp().getRole(); // 获取全局角色

    request.post('/forum/forumComment/publish', {
      forumId: categoryId,
      content: commentContent,
      memberId: memberId, // 传递 memberId
      role: role, // 传递角色
    })
      .then((response) => {
        wx.hideLoading();
...
// 处理回复
onReply(e) {
  const { id, name, forumId } = e.currentTarget.dataset;
  this.setData({
    replyCommentId: id,
...
// 提交回复
publishReply() {
  const { replyContent, replyCommentId, categoryId } = this.data;
