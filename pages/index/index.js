...
Page({
  ...
  onReachBottom() {
    if (this.data.zh_page < this.data.zh_totalPage) {
      this.setData({ zh_page: this.data.zh_page + 1 });
      this.getShopData();
    } else {
  ...
  onPullDownRefresh() {
    this.setData({ store_list: [], zh_page: 1 });
    this.getShopData();
  ...
  scanCode() {
    ...
    if (!token) {
      ...
      setTimeout(() => wx.reLaunch({ url: '../verificationcodelogin/verificationcodelogin' }), 777);
      return;
    }
    wx.scanCode({
      success: (res) => {
        wx.navigateTo({ url: res.result.split('#')[1] });
      },
    });
  },
  openWXSM() {
    ...
    jweixin.scanQRCode({
      needResult: 1,
      success: (res) => {
        wx.navigateTo({ url: res.resultStr.split('#')[1] });
      },
    });
  },
  getAdData() {
    wx.request({
      url: `ad/ad/index`,
      method: 'POST',
      data: { area_list: this.data.address },
      success: (res) => {
        const { status, data } = res.data;
        this.setData({ adData: status.flag ? data : [] });
      },
    });
  },
  getBannerData() {
    ...
    request({
      ...
      const { status, data } = res.data;
      this.setData({ swiperData: status.flag ? data : [] });
    }).catch(err=>{
      console.log(err)
      wx.hideLoading()
    })
  ...
  getNavData() {
    wx.request({
      ...
      success: (res) => {
        const { status, data } = res.data;
        this.setData({ navData: status.flag ? data : [] });
      },
